...
SELECT
	ROUND(MAX(SALARY)) AS 'MAIOR SALÁRIO',
	ROUND(MIN(SALARY)) AS 'MENOR SALÁRIO',
	ROUND(SUM(SALARY)) AS 'SOMA DOS SALÁRIOS',
	ROUND(AVG(SALARY)) AS 'MÉDIA SALARIAL'
FROM employees;


-- Escreva uma query que exiba a quantidade de pessoas que trabalham como pessoas programadoras 
SELECT COUNT(JOB_ID)
FROM employees
GROUP BY JOB_ID
...
SELECT 
	FIRST_NAME,
    LAST_NAME,
    DATEDIFF(CURRENT_DATE(), HIRE_DATE) AS 'DIAS EMPREGADO NA EMPRESA'
FROM employees;
